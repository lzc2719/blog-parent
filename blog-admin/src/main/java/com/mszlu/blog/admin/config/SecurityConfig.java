...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    @Bean
    public BCryptPasswordEncoder bCryptPasswordEncoder(){ //加密策略，相同密码每次加密后的的符号均不同
...
//                .antMatchers("/user/findAll").hasRole("admin") //访问接口需要admin的角色
                ...
                //自定义service 来去实现实时的权限认证。自己写的认证代码，返回true认证通过。
                .antMatchers("/admin/**").access("@authService.auth(request,authentication)")
                .antMatchers("/pages/**").authenticated() //认证通过就可以访问此目录下的页面
                .and().formLogin()
                .loginPage("/login.html") //自定义的登录页面
                .loginProcessingUrl("/login") //登录处理接口
                .usernameParameter("username") //定义登录时的用户名的key 默认为username
                .passwordParameter("password") //定义登录时的密码key，默认是password
                .defaultSuccessUrl("/pages/main.html") //登录成功，跳转至main.html
                .failureUrl("/login.html") //登录失败，跳转至login.html
                ...
                .permitAll() //退出登录的接口放行
                .and()
                .httpBasic() //单纯用postman或网址栏直接登录也会被拦截
                .and()
